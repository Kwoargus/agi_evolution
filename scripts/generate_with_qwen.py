# scripts/generate_with_qwen.py
"""
Генерация концептов через Qwen 2.5 7B.
"""
...
class QwenGenerator:
    """
    Генератор концептов через Qwen 2.5 7B.
    """
    ...
    def generate_concepts(self, category: str, count: int = 10) -> List[Dict]:
        """
        Генерирует концепты для категории через Qwen.
        """
        prompt = f"""
        ...
        try:
            response = requests.post(
                ...
                json={
                    ...
                    "prompt": prompt,
                    "stream": False,
                    "temperature": 0.8,  # Qwen хорошо работает с более высокой температурой
                    "max_tokens": 2000
                },
                timeout=120
            )
    ...
    def build_graph(self, concepts: List[Dict]):
        ...
        for concept in concepts:
            ...
            node_id = f"qwen_{hashlib.md5(name.encode()).hexdigest()[:8]}"
            ...
            node = KnowledgeNode(
                ...
                description=f"Generated by Qwen: {name}"
            )
        ...
        for node_type, type_nodes in by_type.items():
            ...
            for i, node in enumerate(type_nodes):
                ...
                for target in selected:
                    ...
                    edge = KnowledgeEdge(
                        id=f"qwen_edge_{hashlib.md5(f'{node.id}_{target.id}'.encode()).hexdigest()[:8]}",
                        source_id=node.id,
                        target_id=target.id,
...
def main():
    ...
    # Проверяем, запущена ли Qwen
    if not generator.check_model():
        print("❌ Qwen 2.5 7B не запущена!")
        print("   Запустите в другом терминале: ollama run qwen2.5:7b")
        return
    ...
    categories = [
        ...
        "hydraulics", "pneumatics", "thermodynamics"
    ]
    ...
    # Показываем примеры новых узлов от Qwen
    print("\n🔍 ПРИМЕРЫ НОВЫХ УЗЛОВ (Qwen):")
    new_nodes = [n for n in generator.graph.nodes.values() if n.metadata.get("source") == "qwen"]
