# core/evolution.py
"""
[ru] Движок эволюции с поддержкой GAN и визуализацией.
[en] Evolution engine with GAN support and visualization.
"""
...
class EvolutionEngine:
    """
    [ru] Движок эволюции ботов с использованием GAN.
    [en] Bot evolution engine using GAN.
    """
    ...
    def __init__(self, world, population_size: int = 20, generations: int = 50,
        ...
        """
        [ru] Инициализирует движок эволюции.
        Args:
            [ru] world: Объект мира
            [ru] population_size: Размер популяции
            [ru] generations: Количество поколений
            [ru] steps_per_episode: Шагов за эпизод
            [ru] elite_count: Количество элитных особей
            [ru] mutation_rate: Скорость мутации
            [ru] use_gan: Использовать GAN
            [ru] gan_training_epochs: Эпох обучения GAN
            [ru] visualize: Включить визуализацию

        [en] Initializes the evolution engine.
        Args:
            [en] world: World object
            [en] population_size: Population size
            [en] generations: Number of generations
            [en] steps_per_episode: Steps per episode
            [en] elite_count: Number of elite individuals
            [en] mutation_rate: Mutation rate
            [en] use_gan: Use GAN
            [en] gan_training_epochs: GAN training epochs
            [en] visualize: Enable visualization
        """
        self.world = world
        ...
        # ============================================================
        # [ru] ИНИЦИАЛИЗАЦИЯ GAN
        # [en] GAN INITIALIZATION
        # ============================================================
        if use_gan:
            self.gan = GAN(
                latent_dim=128,
            ...
            # ============================================================
            # [ru] ИНИЦИАЛИЗАЦИЯ ИНСТИНКТОВ
            # [en] INSTINCTS INITIALIZATION
            # ============================================================
            try:
                ...
                print(f"[ru] Инстинкты не загружены: {e}")
                print(f"[en] Instincts not loaded: {e}")
        ...
        # ============================================================
        # [ru] ИНИЦИАЛИЗАЦИЯ ВИЗУАЛИЗАЦИИ
        # [en] VISUALIZATION INITIALIZATION
        # ============================================================
        if self.visualize:
            try:
                ...
                print(f"[ru] Визуализация не загружена: {e}")
                print(f"[en] Visualization not loaded: {e}")
    ...
    def _collect_training_data(self, generation: int, fitnesses: List[float],
        ...
        """
        [ru] Собирает данные для визуализации.
        [en] Collects data for visualization.
        """
        ...
        # [ru] Разнообразие паттернов
        # [en] Pattern diversity
        ...
        # [ru] Статистика рефлексов из популяции
        # [en] Reflex statistics from population
        ...
        # [ru] Сохраняем потери GAN, если есть
        # [en] Save GAN losses if available
        if self.gan_stats:
    ...
    def _generate_visualizations(self) -> None:
        """
        [ru] Генерирует все визуализации.
        [en] Generates all visualizations.
        """
        if not self.visualize or self.visualizer is None or self.training_data is None:
            return

        print("\n[ru] Генерация визуализаций...")
        print("\n[en] Generating visualizations...")
        ...
        try:
            ...
            print("[ru] ✅ Все визуализации созданы!")
            print("[en] ✅ All visualizations created!")

        except Exception as e:
            print(f"[ru] Ошибка при генерации визуализаций: {e}")
            print(f"[en] Error generating visualizations: {e}")

    def _generate_and_evaluate_patterns(self, n_patterns: int = 20) -> None:
        """
        [ru] Генерирует и оценивает новые паттерны поведения.
        [en] Generates and evaluates new behavior patterns.
        """
        ...
        try:
            ...
            for pattern in patterns:
                if self.pattern_evaluator:
                    ...
                    if score >= 0.01:  # [ru] ← БЫЛО 0.2, СТАЛО 0.01  [en] ← WAS 0.2, NOW 0.01
                        self.pattern_repository.add_pattern(pattern, score)
                        saved_count += 1
            print(f"[ru]  Сгенерировано {n_patterns} паттернов, сохранено {saved_count}")
            print(f"[en]  Generated {n_patterns} patterns, saved {saved_count}")
        except Exception as e:
            print(f"[ru] Ошибка при генерации паттернов: {e}")
            print(f"[en] Error generating patterns: {e}")


    def _integrate_best_patterns(self) -> None:
        """
        [ru] Интегрирует лучшие сгенерированные паттерны в правила поведения и сохраняет в БД.
        [en] Integrates the best generated patterns into behavior rules and saves to DB.
        """
        ...
        try:
            ...
            from db.connector import save_generated_pattern  # [ru] ← ТОЛЬКО ЭТОТ ИМПОРТ  [en] ← ONLY THIS IMPORT

            # [ru] Текущее поколение (номер, а не ID из БД)
            # [en] Current generation (number, not ID from DB)
            generation_num = len(self.best_fitness_history)  # 1, 2, 3, ...

            new_rules = []
            for i, pattern in enumerate(best_patterns):
                # [ru] Генерируем правило из паттерна
                # [en] Generate rule from pattern
                ...
                try:
                    ...
                    print(f"[ru] Паттерн сохранён в БД: {rule.get('action')} (score: {score:.3f})")
                    print(f"[en] Pattern saved to DB: {rule.get('action')} (score: {score:.3f})")
                except Exception as e:
                    print(f"[ru] Ошибка сохранения паттерна в БД: {e}")
                    print(f"[en] Error saving pattern to DB: {e}")

            # [ru] Добавляем правила в рефлексы лучших особей
            # [en] Add rules to best individuals' reflexes
            ...
            for ind in best_individuals:
                for rule in new_rules[:3]:
                    if rule not in ind.reflex_module.rules:
                        ...
                        print(f"[ru]  Добавлено новое правило рефлекса: {rule.get('action')}")
                        print(f"[en]  Added new reflex rule: {rule.get('action')}")

        except Exception as e:
            print(f"[ru] Ошибка при интеграции паттернов: {e}")
            print(f"[en] Error integrating patterns: {e}")


    def run(self, save_to_db: bool = False) -> List[float]:
        """
        [ru] Запускает эволюцию с детальным логированием GA.
        [en] Runs evolution with detailed GA logging.
        """
        print("\n" + "=" * 70)
        print("[ru] ЗАПУСК ГЕНЕТИЧЕСКОГО АЛГОРИТМА")
        print("[en] STARTING GENETIC ALGORITHM")
        print("=" * 70)
        print(f"[ru]  Популяция: {len(self.population.individuals)} особей")
        print(f"[en]  Population: {len(self.population.individuals)} individuals")
        print(f"[ru]  Поколений: {self.generations}")
        print(f"[en]  Generations: {self.generations}")
        print(f"[ru]  Элита: {self.elite_count} лучших")
        print(f"[en]  Elite: {self.elite_count} best")
        print(f"[ru]  Скорость мутации: {self.mutation_rate}")
        print(f"[en]  Mutation rate: {self.mutation_rate}")
        print("=" * 70 + "\n")

        for gen in range(self.generations):
            start_time = time.time()
            print(f"\n{'=' * 70}")
            print(f"[ru] ПОКОЛЕНИЕ {gen + 1}/{self.generations}")
            print(f"[en] GENERATION {gen + 1}/{self.generations}")
            print(f"{'=' * 70}")

            # ============================================================
            # [ru] 1. ОЦЕНКА ПОПУЛЯЦИИ
            # [en] 1. POPULATION EVALUATION
            # ============================================================
            print("[ru] Оценка особей...")
            print("[en] Evaluating individuals...")
            ...
            print(f"[ru]  Лучший фитнес: {best_fit:.2f}")
            print(f"[ru]  Средний фитнес: {avg_fit:.2f}")
            print(f"[ru]  Худший фитнес: {min_fit:.2f}")
            print(f"[ru]  Собрано переходов: {len(experiences)}")
            print(f"[en]  Best fitness: {best_fit:.2f}")
            print(f"[en]  Average fitness: {avg_fit:.2f}")
            print(f"[en]  Worst fitness: {min_fit:.2f}")
            print(f"[en]  Collected transitions: {len(experiences)}")

            # ============================================================
            # [ru] 2. СОХРАНЕНИЕ В БД (опционально)
            # [en] 2. SAVE TO DB (optional)
            # ============================================================
            if save_to_db:
                ...
                if gen_id:
                    ...
                    print(f"[ru]  💾 Поколение {gen + 1} сохранено в БД (ID: {gen_id})")
                    print(f"[en]  💾 Generation {gen + 1} saved to DB (ID: {gen_id})")

            # ============================================================
            # [ru] 3. СБОР ДАННЫХ ДЛЯ ВИЗУАЛИЗАЦИИ
            # [en] 3. COLLECT DATA FOR VISUALIZATION
            # ============================================================
            if self.visualize:
            ...
            if self.gan and experiences:
                print(f"\n[ru] ОБУЧЕНИЕ GAN на {len(experiences)} переходах...")
                print(f"\n[en] TRAINING GAN on {len(experiences)} transitions...")
                ...
                if len(self.gan.experience_buffer) >= self.gan.batch_size:
                    ...
                    if gan_results and gan_results.get('g_loss') and gan_results.get('d_loss'):
                        ...
                        if gen > 0 and self.gan_stats:
                            ...
                            print(f"[ru] G Loss: {avg_g_loss:.4f} ({g_change:+.1f}% от прошлого)")
                            print(f"[en] G Loss: {avg_g_loss:.4f} ({g_change:+.1f}% from previous)")
                            print(f"[ru] D Loss: {avg_d_loss:.4f} ({d_change:+.1f}% от прошлого)")
                            print(f"[en] D Loss: {avg_d_loss:.4f} ({d_change:+.1f}% from previous)")
                        else:
                            print(f"[ru] G Loss: {avg_g_loss:.4f} (начальный)")
                            print(f"[en] G Loss: {avg_g_loss:.4f} (initial)")
                            print(f"[ru] D Loss: {avg_d_loss:.4f} (начальный)")
                            print(f"[en] D Loss: {avg_d_loss:.4f} (initial)")
                        ...
                        # [ru] Генерируем и оцениваем новые паттерны
                        # [en] Generate and evaluate new patterns
                        self._generate_and_evaluate_patterns()
                        self._integrate_best_patterns()

            # ============================================================
            # [ru] 5. ЭВОЛЮЦИЯ ИНСТИНКТОВ (каждые 5 поколений)
            # [en] 5. INSTINCT EVOLUTION (every 5 generations)
            # ============================================================
            if gen % 5 == 0 and self.use_gan and self.instinct_engine is not None:
                if self.gan and len(self.gan.experience_buffer) > 10:
                    print("\n[ru] ЭВОЛЮЦИЯ ИНСТИНКТОВ...")
                    print("\n[en] INSTINCT EVOLUTION...")
                    try:
                        ...
                        if instinct_history:
                            print(f"[ru]  Лучший фитнес инстинктов: {max(instinct_history):.4f}")
                            print(f"[en]  Best instinct fitness: {max(instinct_history):.4f}")
                            print(f"[ru]  Средний фитнес инстинктов: {np.mean(instinct_history):.4f}")
                            print(f"[en]  Average instinct fitness: {np.mean(instinct_history):.4f}")
                    except Exception as e:
                        print(f"[ru] Ошибка эволюции инстинктов: {e}")
                        print(f"[en] Instinct evolution error: {e}")

            # ============================================================
            # [ru] 6. СОЗДАНИЕ СЛЕДУЮЩЕГО ПОКОЛЕНИЯ (GA)
            # [en] 6. CREATE NEXT GENERATION (GA)
            # ============================================================
            if gen < self.generations - 1:
                print("\n[ru] СОЗДАНИЕ СЛЕДУЮЩЕГО ПОКОЛЕНИЯ (GA)...")
                print("\n[en] CREATING NEXT GENERATION (GA)...")

                # [ru] Выбираем элиту
                # [en] Select elite
                best_individuals = self.population.select_best(self.elite_count)
                print(f"[ru]  Элита: {len(best_individuals)} особей, средний фитнес: {np.mean([ind.fitness for ind in best_individuals]):.2f}")
                print(f"[en]  Elite: {len(best_individuals)} individuals, average fitness: {np.mean([ind.fitness for ind in best_individuals]):.2f}")

                # [ru] Создаём новое поколение
                # [en] Create new generation
                self.population.next_generation(
                    elite_count=self.elite_count,
                    mutation_rate=self.mutation_rate
                )

                # [ru] Логируем изменения в геномах
                # [en] Log genome changes
                ...
                if old_best and hasattr(old_best, 'genome') and hasattr(new_best, 'genome'):
                    ...
                    if changes:
                        print(f"[ru] Изменения в геноме лучшей особи:")
                        print(f"[en] Changes in best individual's genome:")
                        for change in changes[:5]:
                            print(f"[ru]    • {change}")
                            print(f"[en]    • {change}")
                    else:
                        print(f"[ru] Геном лучшей особи не изменился (мутации не произошло)")
                        print(f"[en] Best individual's genome unchanged (no mutation)")

                print(f"[ru]  Популяция: {len(self.population.individuals)} особей")
                print(f"[en]  Population: {len(self.population.individuals)} individuals")

            elapsed = time.time() - start_time
            print(f"\n[ru] Время поколения: {elapsed:.2f} сек.")
            print(f"\n[en] Generation time: {elapsed:.2f} sec.")

        # [============================================================
        # [ru] 7. ФИНАЛЬНАЯ СТАТИСТИКА
        # [en] 7. FINAL STATISTICS
        # ============================================================
        print("\n" + "=" * 70)
        print("[ru] ЭВОЛЮЦИЯ ЗАВЕРШЕНА")
        print("[en] EVOLUTION COMPLETED")
        print("=" * 70)
        print(f"[ru]  Лучший фитнес за всё время: {max(self.best_fitness_history):.2f}")
        print(f"[en]  Best fitness overall: {max(self.best_fitness_history):.2f}")
        print(f"[ru]  Финальный фитнес: {self.best_fitness_history[-1]:.2f}")
        print(f"[en]  Final fitness: {self.best_fitness_history[-1]:.2f}")
        print(f"[ru]  Всего поколений: {len(self.best_fitness_history)}")
        print(f"[en]  Total generations: {len(self.best_fitness_history)}")
        ...
        if self.gan_stats:
            ...
            print(f"\n[ru]  Финальные потери GAN:")
            print(f"\n[en]  Final GAN losses:")
            print(f"[ru] G Loss: {final_g:.4f}")
            print(f"[en] G Loss: {final_g:.4f}")
            print(f"[ru] D Loss: {final_d:.4f}")
            print(f"[en]  D Loss: {final_d:.4f}")

        print("=" * 70 + "\n")

        # [ru] Финальная визуализация
        # [en] Final visualization
        if self.visualize:
