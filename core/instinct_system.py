...
@dataclass
class SensoryInput:
    # [ru] векторное представление того, что видит бот
    # [en] vector representation of what the bot sees
    vision: np.ndarray
    # [ru] векторное представление того, что слышит бот
    # [en] vector representation of what the bot hears
    sound: np.ndarray
    # [ru] векторное представление того, что чувствует бот
    # [en] vector representation of what the bot smells
    smell: np.ndarray
    # [ru] позиция объекта
    # [en] object position
...
class InstinctSystem:
    """
    [ru] Система инстинктов с мультимодальным восприятием.
    [en] Instinct system with multimodal perception.
    """
    ...
    def perceive(self, sensory_input: SensoryInput) -> np.ndarray:
        """
        [ru] Преобразует сенсорные данные в единый вектор.
        [en] Converts sensory data into a single vector.
        """
    ...
    def get_best_action(self, sensory_input: SensoryInput) -> Optional[InstinctAction]:
        """
        [ru] Находит лучшее действие на основе восприятия.
        [en] Finds the best action based on perception.
        """
        ...
        for pattern in self.instinct_patterns:
            # [ru] Вычисляем схожесть с паттерном
            # [en] Compute similarity with the pattern
    ...
    def _compute_similarity(self, vec1: np.ndarray, vec2: np.ndarray) -> float:
        """
        [ru] Вычисляет косинусное сходство.
        [en] Computes cosine similarity.
        """
